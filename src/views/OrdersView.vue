        ...
        <tr v-for="order, index in renderOrders" :key="index">
          ...
          <td>
            <router-link :to="{name: 'orderView' , params:{id:order.id}}" >
              <span class="btn btn-success">Show Details</span>
            </router-link>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      filteredOrders( ) {
        let orders = [];
        orders = (this.selectedStatus || this.queryName.length || this.queryTel.length)
        ...
        this.$store.commit('ordersSearch', orders);
      },
